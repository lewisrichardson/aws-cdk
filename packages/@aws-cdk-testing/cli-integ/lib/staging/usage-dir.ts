...
import { addToFile, copyDirectoryContents } from '../files';

export const DEFAULT_USAGE_DIR = path.join(os.homedir(), '.codeartifact/usage');
...
export async function prepareUsageDir() {
  ...
  await fs.writeFile(path.join(dir, 'activate.bash'), [
    `for line in $(cat ${dir}/env); do export $line; done`,
